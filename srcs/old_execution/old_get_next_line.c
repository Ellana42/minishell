...
/*   Updated: 2022/12/06 12:36:05 by lsalin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_read_line(int fd, char **keep, char **tmp)
{
	...
	while (r > 0)
	{
		...
		if (r == -1)
		{
			free_three_strs(&buf, keep, tmp);
			return ;
		}
		buf[r] = '\0';
		*tmp = ft_strdup(*keep);
		free_three_strs(keep, 0, 0);
		*keep = join_strs(*tmp, buf);
		free_three_strs(tmp, 0, 0);
		if (contains_newline(*keep))
			break ;
	}
	free_three_strs(&buf, 0, 0);
}
